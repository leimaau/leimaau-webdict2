...
MainQuery = (() => {
	...
	tempObj.queryTableOne_triungkox = (searchValue, selVal, queryType) => {
		...
		if (queryType == 'char' || queryType == 'char_simp') { // 查詢繁簡體
			querySQL += ` where word1 = '${searchValue}' or word2 = '${searchValue}' order by ID`;
		} else if (queryType == 'jyutping') { // 查詢無調拼音
			querySQL += ` where pinyin = '${searchValue}' or pinyin GLOB '${searchValue}[xh]' order by ID`;
		} else if (queryType == 'jyut6ping3') { // 查詢有調拼音
			querySQL += ' where pinyin = ? order by ID';
		} else if (queryType == 'expl') { // 詞例反查
	...
	tempObj.queryTableOne_gw = (searchValue, selVal, queryType) => {
		...
		if (queryType == 'char' || queryType == 'char_simp') { // 查詢繁簡體
			...
			querySQL += ` where jyutping GLOB '${searchValue}[1-8]' or jyutping = '${searchValue}7°' order by ID`;
		} else if (queryType == 'jyut6ping3') { // 查詢有調粵拼
			querySQL += ` where jyutping = ? or jyutping = '${searchValue}°' order by ID`;
		} else if (queryType == 'expl') { // 詞例反查
			querySQL += ` where expl GLOB '*${searchValue}*' order by ID`;
		};
		return DictDb.execParam( querySQL, (queryType == 'jyutping' || queryType == 'expl') ? [] : [searchValue] );
	...
	tempObj.queryTableOne_jw = (searchValue, selVal, queryType) => {
		let querySQL = `select '${selVal[0]}' YEAR,* from ${selVal[0]}`;
		if (queryType == 'char' || queryType == 'char_simp') { // 查詢繁簡體
			querySQL += ` where word GLOB '*${searchValue}*' or word_comp GLOB '*${searchValue}*' order by ID`;
		} else if (queryType == 'jyutping') { // 查詢無調粵拼
			querySQL += ` where jyutping GLOB '${searchValue}[1-8]' or jyutping = '${searchValue}7°' order by ID`;
		} else if (queryType == 'jyut6ping3') { // 查詢有調粵拼
			querySQL += ` where jyutping = ? or jyutping = '${searchValue}°' order by ID`;
