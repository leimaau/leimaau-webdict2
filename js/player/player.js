(function($){
	...
	if (isShuffle && shuffle_array != undefined && playlist.length === (shuffleArray=JSON.parse(shuffle_array)).length) {
		...
		shuffleIndex = 0;
	    $('#QPlayer .cover').attr('title', '點擊關閉隨機播放');
	} else {
		isShuffle = false;
	    $('#QPlayer .cover').attr('title', '點擊開啓隨機播放');
	}
	...
	$("#QPlayer .ssBtn").on('click', function(){
		...
		if ($('.ssBtn .adf').hasClass('on') === false) {
			if (isFirstPlay) {
			    setTimeout("showTips('#player .cover','點擊封面開啓(關閉)隨機播放', " + function(){setTimeout("showTips('#player .ctrl .musicTag','點擊拖動標題欄快進(快退)')", 1000)} + ");", 500);
	...
	$("#player .cover").on('click',function(){
		isShuffle = !isShuffle;
		if (isShuffle) {
	        $("#player .cover").attr("title","點擊關閉隨機播放");
	        showNotification('已開啓隨機播放');
	        ...
	        $("#player .cover").attr("title","點擊開啓隨機播放");
	        showNotification('已關閉隨機播放');
...
//檢測標題和作者信息總寬度是否超出播放器，超過則返回true開啓跑馬燈
